# Resumo - Aula 01: Apresentação da Disciplina e Introdução ao DevOps

**Disciplina:**  Integração e Entrega Continua (DevOps)

**Instituição:** FATEC Sorocaba (Faculdade de Tecnologia)  
**Professor:** Prof. Deivison S. Takatu (`deivison.takatu@fatec.sp.gov.br`) 

---

## 1. Lista de Conteúdo da Aula 01

-Apresentação da Disciplina e Contextualização;

-Apresentação

-Plano de Aulas e Ensino;

-Contexto da Disciplina;

-Sequência dos Conteúdos do Semestre;

-Metodologia de Ensino;

-Critérios de Avaliação;

-Atividade;

-Conclusão.




## 2. Visão Geral Aula
O professor apresentou sobre o seu histórico acadêmico e orientou sobre os métodos de avaliação para o semestre, além de reforçar sobre CI/CD (integração contínua e entrega contínua) e o que aprendemos durante o semestre anterior.

---

## 3. Conteúdo: Introdução ao Versionamento e Deploy

A matéria de Programação Web tem como foco construir interfaces e desenvolver aplicações/projetos. Em conjunto com a disciplina de Deploy, é possível realizar o versionamento, registro e monitoramento das versões de desenvolvimento com mais facilidade. Realizar esses registros permite:

- Recuperar versões
  
- Realizar projetos colaborativos

- Automação

- Deploy

- Controle de versões

---

## 3. Conteúdo: Automação de Pipelines (CI/CD)
Foi apresentado também o conceito de pipelines, que seria a sequência automatizadas de tarefas, onde atividades que são repetitivas podem ser otimizadas, diminuindo retrabalho e também impedindo que falhas humanas passem despercebidas. 

---

## 4. Conteúdo: Integração Contínua (Continuous Integration - CI)

Quando realizamos a alteração, utilizando CI fica muito mais fácil de identificar bugs e problemas logo no início, diminuindo os conflitos que poderiam acontecer durante o restante do desenvolvimento. efmwfk


---
## 5. Conteúdo: Qualidade de Software e Testes Automatizados

O nível de qualidade do projeto deve ser validado durante todo o  processo de desenvolvimento, então é uma opção viável utilizar ferramentas de automação que analisam: Padronização do código, segurança, complexidade, cobertura de testes, vulnerabilidades.

Quanto mais rápido conseguirmos identificar uma falha ou bug menor será o prejuízo.


## 6. Entrega Contínua (Continuous Delivery - CD)

Com a entrega contínua, após todas as alterações serem aprovadas e estiverem funcionando corretamente as modificações são transferidas para a etapa de deploy, que de forma padronizada irá realizar esse processo sem causar riscos e também otimizando tempo.

---

## 7. Conteúdo: Containers e Estratégias de Implantação

Os containers servem para padronizar ambientes, então caso realize o desenvolvimento com uma tecnologia que possui versões diferentes que podem se comportar de outra maneira dependendo do dispositivo. Dessa forma, o container vai permitir que eles funcionem da mesma maneira independente da máquina, local, teste e produção. O Docker é um dos softwares mais conhecidos e amplamente utilizado no mercado, além de ser utilizado frequentemente em outras disciplinas.

## 8. Conteúdo: Importância do Portfólio Profissional

No conteúdo da aula é apresentada a importância de ter um portfolio para demonstrar as habilidades, projetos e experiência do aluno de uma forma centralizada, facilitando para que a pessoas conheçam o que foi produzido durantes as aulas. Ter um portfólio pode ajudar amplamente em conseguir uma vaga na área, ajudando o avaliador a conhecer o seu trabalho. 

---

## 10. Atividade da Aula 01
1. **Formação de Grupos:** Grupos fixos de 3 a 5 integrantes para a realização das atividades do semestre

 Grupo: Karina de Moraes, Mike Willy, Nicolly Lemos, Pedro Henrique e Rafaela Mansano

2. **Repositório GitHub:** Criação do repositório principal do grupo contendo o resumo da Aula 01 

 Link do repositório: https://github.com/nicollylemos/AtividadesDevOps/tree/main/Aula%2001%20-%20Apresenta%C3%A7%C3%A3o%20da%20Disciplina%20e%20Introdu%C3%A7%C3%A3o%20ao%20DevOps

3. **Avaliação Diagnóstica:** Preenchimento da avaliação diagnóstica inicial da aula:

 Preenchido! Respondi no dia então não tenho print.

---

## 11. Referências Bibliográficas
 
 Deivison S. Takatu AULA 01 - APRESENTAÇÃO DA DISCIPLINA E INTRODUÇÃO AO DEVOPS. Aula 01 - Apresentação da Disciplina e Introdução ao DevOps. Disponível em: <https://docs.google.com/presentation/d/1Eg_tgGa3A9m7DWAX5t2mih00tzql9jYj0KA9otS6kQI/edit?slide=id.g3f661a119ed_0_23#slide=id.g3f661a119ed_0_23>. Acesso em: 13 ago.. 2026.
‌
